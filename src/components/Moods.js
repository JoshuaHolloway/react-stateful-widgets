...
export default function Moods() {
  ...
  const moods = {
    happy: 'quite happy!',
    sad: 'rather sad',
    not_sure: 'not sure how I feel',
  };

  const [mood, setMood] = useState(moods['not_sure']);    /* STEP 1 */

    const makeHappy = () => setMood(moods['happy']);    /* STEP 4 */
    const makeSad   = () => setMood(moods['sad']);      /* STEP 5 */
    const reset     = () => setMood(moods['not_sure']); /* STEP 6 */

  const style = {
    fontSize: '1.5em',
    marginBottom: '0.3em',
    color: mood === moods['happy'] ? 'royalblue' : 'crimson', /* STEP 2 */
  };
