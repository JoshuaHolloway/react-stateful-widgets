...
export default function Programmers() {
  ...
  const [slice1, set_slice1] = useState(listOfAwesome);
  const [slice2, set_slice2] = useState(null);

  const getNameOfFeatured = () => {
    // It's going to utilize both slices of state to return the _name_ of the featured dev.
    // The beauty of closures is that we can "see" both slices of state from this region
    // of the program, without needing to inject the information through arguments.
    const programmer = slice1[slice2-1];
    // console.log('slice2: ', slice2);
    // console.log('programmer: ', slice1[slice2-1], '[slice2.id-1]=', slice2-1);
  ...
  const style = {
    ...
    color: slice2 === null ? 'royalblue' : 'gold'
  };
